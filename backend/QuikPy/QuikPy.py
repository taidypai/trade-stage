...
class QuikPy:
    ...
    def close_connection_and_thread(self):
        """Закрывает соединение для запросов и поток обработки функций обратного вызова"""
        # Проверяем существование атрибута перед использованием
        if hasattr(self, 'callback_exit_event') and self.callback_exit_event:
            self.callback_exit_event.set()  # Останавливаем поток обработки функций обратного вызова

        if hasattr(self, 'socket_requests') and self.socket_requests:
